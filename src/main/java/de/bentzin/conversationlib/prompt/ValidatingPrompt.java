...
public abstract class ValidatingPrompt implements Prompt {
    ...
    @Override
    @Nullable
    public Prompt getNextPrompt(@NotNull ConversationContext context, @Nullable String input) {
        if (isInputValid(context, input)) {
            return acceptValidatedInput(context, input);
        } else {
            Component failPrompt = getFailedValidationText(context, input);
            if (failPrompt != null) {
                context.target().sendMessage(failPrompt);
            }
